...
namespace y
{
	...
	namespace math
	{
		...
		template <int len>
		typename cipher<len>::encoded_sequence_t cipher<len>::encrypt( const encoded_sequence_t & msg ) const
		{
		...
		template <int len>
		typename cipher<len>::encoded_sequence_t cipher<len>::encode( const std::string & msg )
		{
			if(msg.empty())
				return encoded_sequence_t();
			encoded_sequence_t encoded(1 + msg.size()/(3*len) + 1); // encode 3 chars into one int
		...
		template <int len>
		typename decipher<len>::encoded_sequence_t decipher<len>::decrypt( const encoded_sequence_t & msg ) const
		{
			encoded_sequence_t m(msg);
			for(auto& i : m)
				i = i.mod_pow(d, n);
		...
		void generate_keys(cipher<len>& pbk, decipher<len>& pvk)
		{
			...
			while(true){
				std::cout << " ------- locating prime numbers p and q ... ------- " << std::endl;
				...
				for(int i = 0; i < 100; i++){
					...
					std::cout << "e*d mod phi: " << std::endl << e.mod_mult(d, phi) << std::endl;
